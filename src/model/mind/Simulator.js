...
(function() {
			...
			for(var i = 0; i < driversList.length; i++) {
				var commandDrivers = driversList[i].drivers;
					...
					commandDrivers.sort(function(a, b) {
						//影響度の大きい順にソート
						return b.getReview() - a.getReview();
					});
			}
			...
			for(var i = 1; i < maxNumberOfJoint; i++) {
					...
					for(var j = 0; j < commandArrangements.length; j++) {
						var　arrangements = commandArrangements[j];
							...
							chooseEachCommandDriverPairs(0, numberOfJoint, arrangements, destinations, [], destinationArrangements);
							result = result.concat(destinationArrangements);
					}
			}
	...
	p._checkInTest = function(data, review_func, options) {
		var self = this;
			...
			var parameters = self._parameters;
			...
			var targetFamily = self._pickUpElements(
					...
					parameters.familyMemberNames,
					targets,
					options
					);
			var targetEnemy = self._pickUpElements(
					parameters.memberKey,
					parameters.enemyMemberNames,
					targets,
					options
					);
